...
If we let \(u=1-4x\) , then \(du=-4dx\) . We multiply the integrand by
\(\frac{-4}{-4}\). Thus \[
\int(1-4x)^{1/2} \,dx = \int(1-4x)^{1/2} \cdot \frac{-4}{-4} dx = \int u^{1/2}( -\frac{du}{4}) \ = -\frac{1}{4}\int u^{1/2} du = -\frac{1}{4} \cdot \frac{2u^{3/2}}{3} + C
\]
...
\subsubsection{5.6.2 L'Hospital's Rule}\label{lhospitals-rule}}
...
\displaystyle\lim\emph{\{x \to 0\}
\frac{\sin5x}{\sin3x}=\displaystyle\lim}\{x \to 0\}
\frac{5\cos5x}{3\cos3x}=\frac{5}{3} \$\$
...
\displaystyle\lim\emph{\{x
\to +\infty\}\frac{3x-1}{7-6x}=\displaystyle\lim}\{x \to +\infty\}
\frac{3}{-6}=-\frac{1}{2} \$\$
...
\to k\}\log\emph{a{[}f(x){]}=\displaystyle\lim}\{y \to 0\^{}+\}\log\_a
y=+\infty. \$\$
...
\emph{Remark 5.6.8.} L'Hospital's Rule works only for indeterminate
forms of type \(\frac{0}{0}\) and \(\frac{\infty}{\infty}\). Any other
...
\leavevmode\vadjust pre{\hypertarget{htm:definition}{}}%
\emph{Definition 5.6.10} Let \(f\) be a nonconstant function. The
...
\leavevmode\vadjust pre{\hypertarget{htm:remark}{}}%
\emph{Remark 5.6.11} If \(\displaystyle\lim_{x\to a}{f(x)^{g(x)}}\) is
...
::: \{\#exm: example\} \emph{Example 5.6.12} Evaluate the following
limits.
